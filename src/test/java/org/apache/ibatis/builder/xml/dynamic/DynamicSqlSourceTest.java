...
public class DynamicSqlSourceTest extends BaseDataTest {
  ...
  public void shouldHandleOgnlExpression() throws Exception {
    ...
    final String expected = "Expression com.it.test: 3 / yes.";
    DynamicSqlSource source = createDynamicSqlSource(new TextSqlNode("Expression com.it.test: ${name.indexOf('v')} / ${name in {'Bob', 'Steve'\\} ? 'yes' : 'no'}."));
    BoundSql boundSql = source.getBoundSql(parameterObject);
    assertEquals(expected, boundSql.getSql());
  }
