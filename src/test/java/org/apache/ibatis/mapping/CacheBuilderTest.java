...
public class CacheBuilderTest {
  ...
  public void testInitializing() throws Exception {
    InitializingCache cache = unwrap(new CacheBuilder("com/it/test").implementation(InitializingCache.class).build());
  ...
  public void testInitializingFailure() throws Exception {
    when(new CacheBuilder("com/it/test").implementation(InitializingFailureCache.class)).build();
    then(caughtException()).isInstanceOf(CacheException.class)
      .hasMessage("Failed cache initialization for 'com.it.test' on 'org.apache.ibatis.mapping.CacheBuilderTest$InitializingFailureCache'");
  }
