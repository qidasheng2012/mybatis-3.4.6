...
if [ $TRAVIS_REPO_SLUG == "mybatis/mybatis-3" ] && [ "$TRAVIS_PULL_REQUEST" == "false" ] && [ "$TRAVIS_BRANCH" == "master" ] && [[ "$commit_message" != *"[maven-release-plugin]"* ]]; then
  ...
  if [ $TRAVIS_JDK_VERSION == "oraclejdk8" ]; then
    ...
    # Deploy to sonatype
    ./mvnw clean deploy -Dmaven.com.it.test.skip=true -q --settings ./travis/settings.xml
    echo -e "Successfully deployed SNAPSHOT artifacts to Sonatype under Travis job ${TRAVIS_JOB_NUMBER}"

    ./mvnw clean com.it.test jacoco:report coveralls:report -q --settings ./travis/settings.xml
